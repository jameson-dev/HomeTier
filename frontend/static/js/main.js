...
async function loadDashboardData() {
    try {
        ...
        document.getElementById('new-devices-count').textContent = newDevices;
        
        // Update last scan time
        if (devices.length > 0) {
            const lastScan = Math.max(...devices.map(d => new Date(d.last_seen)));
            document.getElementById('last-scan-time').textContent = formatDateTime(lastScan);
            document.getElementById('last-scan-display').textContent = formatDateTime(lastScan);
        }
